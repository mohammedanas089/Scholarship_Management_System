...
public partial class adminrenew : System.Web.UI.Page
{
    ...
    void PopulateGridview()
    {
        ...
        if (!String.IsNullOrWhiteSpace(tb.Text))
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select * from renewtbl" +
                " where type='R' and stat='Unapproved' and ((name like '%" + tb.Text + "%' ) or (course '%" + tb.Text + "%') or (aadhar like '%" + tb.Text + "%' ))", sql);
        ...
        else if (String.IsNullOrWhiteSpace(tb.Text))
        {
            ...
            sql.Open();
            SqlDataAdapter sda = new SqlDataAdapter("select * from cantbl where type='R' and stat='Unapproved'", sql);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            GridView1.DataSource = dt;
            GridView1.DataBind();
    ...
    protected void GridView1_RowCommand1(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "approve")
        {
            ...
            sql.Open();
            // Label2.Text = "2";
            SqlCommand cmd3 = new SqlCommand("select email from cantbl where aadhar=" + Convert.ToInt64(e.CommandArgument.ToString()) , sql);
            ...
            try
            {
                ...
                // SqlCommand cmd = new SqlCommand("insert into aprvtbl(income,id,name,father,occupation,course,courseyear,pcourse," +
                //       "pcourseyear,pcoursemarks,sslc,pu,institute,balancefee,year,email,aadhar,type) select income,aadhar,name,father,occupation,course,courseyear,pcourse,pcourseyear,pcoursemarks,sslc,pu,institute,balancefee,year,email,aadhar,type from renewtbl where id=" + Convert.ToInt64(e.CommandArgument.ToString()), sql);
                //cmd4.Parameters.AddWithValue("@id", Convert.ToInt64( Label1.Text));
                //cmd.Parameters.AddWithValue("@aadhar",e.CommandArgument.ToString());
                SqlCommand cmd = new SqlCommand("update cantbl set stat='approved' where aadhar=" + Convert.ToInt64(e.CommandArgument.ToString()), sql);
                ...
                sql.Close();
                //    Label2.Text = "9";
                Response.Redirect("adminrenew.aspx");
            }
            catch (Exception ex)
            {
        ...
        if (e.CommandName == "deny")
        {
            ...
            Response.Redirect("reject.aspx?id=" + e.CommandArgument.ToString() + "&type='R'");
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        if (!String.IsNullOrWhiteSpace(tb.Text))
        {
            ...
            SqlConnection sql = new SqlConnection(con);//makes connection to sql
                                                       //selects product as given in searchbox and displays only them
            SqlDataAdapter sda = new SqlDataAdapter("select * from cantbl where (type='R') and (stat='Unapproved') and ((name like '%" + tb.Text + "%' ) or (aadhar like '%" + tb.Text + "%' ))", sql);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            GridView1.DataSource = dt;
            GridView1.DataBind();
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Int64 f = Convert.ToInt64(((Label)e.Row.FindControl("fee")).Text);
    ...
    protected void Button4_Click(object sender, EventArgs e)
    {
        ...
        cmd.Parameters.AddWithValue("@aadhar", r1.Cells[18].Text);
        cmd.ExecuteNonQuery();
        Response.Redirect("adminrenew.aspx");
    }
